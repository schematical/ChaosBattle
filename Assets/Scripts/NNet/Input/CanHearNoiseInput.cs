using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class CanHearNoiseInput : InputNeuron
{
    public int pitch;

    public CanHearNoiseInput(string id) : base(id)
    {
        
    }

    public override void Evaluate()
    {
        _lastValue = 0f;

        foreach(WorldEvent worldEvent in NpcnNet.tickEvents){
            if(worldEvent.eventType == WorldEvent.WorldEventTypes.NOISE_ENTER){
                WorldNoiseEvent worldNoiseEvent = (WorldNoiseEvent)worldEvent;
                if(worldNoiseEvent.noiseObject.pitch == pitch){
                    /*if(_lastValue.Equals(0f)){
                        _lastValue = 0f;
                    }*/
                    _lastValue += worldNoiseEvent.noiseObject.volume;
                }
               
            }
        }

       
    }
    public override NNetData.NeuronData ToData()
    {
        NNetData.NeuronData neuronData = base.ToData();
        neuronData.Set("pitch", pitch.ToString());
        return neuronData;
    }
    public override void ParseData(NNetData.NeuronData neuronData)
    {
        base.ParseData(neuronData);
        pitch = int.Parse(neuronData.Get("pitch"));
    }
    public override void PopulateRandom()
    {
        base.PopulateRandom();
        int index = Random.Range(0, 7);
    
    }

}